...
if [[ "${1:-}" == "--pack" ]]; then
    build_webui
    pack_module "$ROOT/dist/TextMenuCleaner-Global-1.4.zip"
    echo "Built $ROOT/dist/TextMenuCleaner-Global-1.4.zip"
    exit 0
fi
...
PY
cp "$WORK/patched.patch.json" "$ROOT/dist/TextMenuCleaner-Global-1.4.patch.json"
build_webui
pack_module "$ROOT/dist/TextMenuCleaner-Global-1.4.zip"
echo "Built $ROOT/dist/TextMenuCleaner-Global-1.4.zip"
